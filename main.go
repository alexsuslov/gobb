...
import (
	...
	"github.com/alexsuslov/gobb/api/controllers"
	"github.com/alexsuslov/gobb/api/graphql"
	"github.com/alexsuslov/gobb/api/models"
	"github.com/alexsuslov/gobb/api/utils"
	"github.com/alexsuslov/gobb/pkg/config"
	"github.com/gorilla/mux"
	"gitlab.42do.ru/prefix/service/pkg/serve"
	"go/build"
	"net/http"
	"path/filepath"
)
...
func main() {
	...
	schema, err := gobbgraphql.GetSchema()
	if err != nil {
		panic(err)
	}

	r.HandleFunc("/grail", graphiql.ServeGraphiQL)
	r.HandleFunc("/graphql", serve.ServeGraphQL(schema))
